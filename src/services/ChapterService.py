...
class ChapterService:
    ...
    async def create_new_chapter(self,course_id,session:AsyncSession):
        course= await course_service.get_course_by_id(course_id,session)
        if not course:
    ...
    async def delete_chapter(self,chapter_id:UUID,session:AsyncSession):

        chapter=await self.get_chapter_by_id(chapter_id=chapter_id,session=session)

        course= await course_service.get_course_by_id(chapter.course_id,session)
